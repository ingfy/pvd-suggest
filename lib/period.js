...
Period.prototype.countYears = function () {
    return this.end.year - this.start.year + 1;
};

Period.prototype.getMonthsWithNr = function (nr) {
    var out = [];

    var month = new Month(this.start.year, this.start.nr);

    while (this.includes(month)) {
        if (month.nr === nr) {
            out.push(month);
            month = month.next(12);
        } else {
